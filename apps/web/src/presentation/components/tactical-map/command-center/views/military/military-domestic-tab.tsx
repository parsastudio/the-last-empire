...
export function MilitaryDomesticTab({
  ...
  const t = useTranslations("military.domesticTab");
  const gdp = getNationGdp(nation, provincesMap);

  return (
    <div className="space-y-5 animate-fade-smooth text-start">
      <div className="p-3.5 bg-secondary/30 border border-border/60 rounded-2xl flex items-center justify-between">
        <h3 className="text-xs font-black text-foreground">{t("header")}</h3>
      </div>
