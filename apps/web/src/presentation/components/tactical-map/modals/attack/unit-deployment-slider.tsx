...
export function UnitDeploymentSlider({
  ...
  return (
    <div className="bg-secondary/40 border border-border/70 p-4 rounded-3xl space-y-3 font-sans dir-rtl text-right hover:border-primary/40 transition-all shadow-md backdrop-blur-xl relative overflow-hidden group">
      <div className="flex items-center justify-between text-xs">
        <div className="flex items-center gap-2.5">
          <div className="p-2 rounded-2xl bg-secondary/90 border border-border/60 shadow-inner">
            <Icon size={16} className={iconColorClass} />
          </div>
          <span className="font-black text-foreground text-xs">{label}</span>
        </div>

        <div className="flex items-center gap-1.5 font-mono text-[11px] bg-background/80 border border-border/60 px-3 py-1 rounded-xl shadow-inner">
