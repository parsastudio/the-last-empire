...
export class MapShader {
  ...
  public static applyShading(
    ...
    const palette = TacticalMapProfiler.profilePhase("1.PaletteGen", () =>
      this.paletteGenerator.generatePalette(countries),
    );

    const staticCache = TacticalMapProfiler.profilePhase("2.StaticCache", () =>
      this.staticCacheBuilder.buildOrGetCache(maskData, width, height),
    );

    let dynamicIds: Uint16Array | null = null;

    TacticalMapProfiler.profilePhase("3.DynamicGrid", () => {
    ...
    const landLut32 = TacticalMapProfiler.profilePhase("4.LutBuild", () => {
      const lut = new Uint32Array(250 * 3);
    ...
    TacticalMapProfiler.profilePhase("5.PixelLoop", () => {
