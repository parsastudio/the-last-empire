...
function TradeActionForm({
  ...
  nationId,
  onConfirm,
}: Omit<TradeActionDialogProps, "isOpen" | "onClose"> & { nationId: string }) {
  const safeMax = Math.max(0, maxAmount);
  const initialAmount =
    safeMax <= 0
      ? 0
      : Math.min(safeMax, Math.max(1, Math.floor(safeMax * 0.25)));
      ...
      <div className="space-y-3 font-mono text-xs">
        ...
        <div className="bg-secondary/40 p-3.5 rounded-2xl space-y-2 text-[11px] border border-border/60">
          ...
          <div className="flex justify-between items-center font-bold border-t border-border/60 pt-2 text-xs">
            ...
            <span className={`text-sm ${isBuy ? "text-rose-500" : "text-gdp"}`}>
...
export function TradeActionDialog({
  ...
  const isBuy = mode === "buy";
  const subLabel = resourceName.includes("نفت")
  ...
  return (
    ...
    >
      <TradeActionForm
        key={`${mode}-${resourceName}-${maxAmount}-${nationId}`}
