...
import { useWideProxyForm } from "@/presentation/components/tactical-map/command-center/views/hooks/use-wide-proxy-form";
...
export function WideProxyView({
  ...
  const form = useWideProxyForm({ nation, nationsMap, selectedTargetCode });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 dir-rtl text-right">
        <div className="lg:col-span-5 space-y-3 bg-background/30 p-4 border border-border/60 rounded-3xl">
          ...
          <div className="space-y-1.5 max-h-[380px] overflow-y-auto pr-1 scrollbar-thin">
            {form.countryOptions.map((target) => {
              const isSelected = target.id === form.selectedTargetId;
              const flag = getFlagEmoji(target.flagCode);
              return (
                <button
                  key={target.id}
                  onClick={() => form.setSelectedTargetId(target.id)}
        ...
        <div className="lg:col-span-7 space-y-4 bg-background/40 p-5 border border-border/80 rounded-3xl">
          {form.selectedTargetNation ? (
            <>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-sm font-extrabold text-foreground">
                      عملیات نفوذ پنهان علیه {form.selectedTargetNation.name}
                    </h3>
                    <span className="text-[10px] text-muted-foreground font-mono">
                      ثبات سیاسی فعلی:{" "}
                      {PersianNumberFormatter.toPersianDigits(
                        form.selectedTargetNation.stability,
                      )}
                      ٪ | تولید ناخالص:{" "}
                      {PersianNumberFormatter.formatCurrency(
                        form.selectedTargetNation.gdp,
                      )}
                    </span>
                  </div>
                </div>
              ...
              <div className="space-y-4 font-mono text-xs">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-bold text-military text-sm">
                    -{PersianNumberFormatter.toPersianDigits(form.desiredDrain)}
                    ٪
                  </span>
                </div>
                ...
                <input
                  ...
                  max="15"
                  step="1"
                  value={form.desiredDrain}
                  onChange={(e) => form.setDesiredDrain(Number(e.target.value))}
                ...
                <PercentageSelector
                  ...
                  onSelect={(val) => form.setDesiredDrain(val)}
              ...
              <button
                onClick={form.handleFundProxy}
                disabled={
                  form.requiredBudget <= 0 ||
                  !form.canAfford ||
                  form.isSubmitting
                }
                ...
                <span>
                  {form.isSubmitting
                    ...
                    : !form.canAfford
                      ...
                      : `اجرای عملیات و کاهش -${PersianNumberFormatter.toPersianDigits(form.desiredDrain)}٪ ثبات ${form.selectedTargetNation.name}`}
                </span>
              </button>
            </>
          ) : (
